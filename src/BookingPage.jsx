      ...
      {(shop?.address || shop?.business_hours || shop?.phone || shop?.whatsapp) && (
        <div style={{ background:BT.surface, borderBottom:`1px solid ${BT.border}`, padding:"0.75rem 1.5rem", display:"flex", flexWrap:"wrap", gap:"0.75rem 1.5rem" }}>
          ...
          {(shop?.whatsapp || shop?.phone) && (() => {
            ...
            const href    = shop.whatsapp
              ? `https://wa.me/55${digits}`
              : `tel:${digits}`;
            return (
              <a href={href} target={shop.whatsapp ? "_blank" : undefined} rel="noopener noreferrer"
                style={{ display:"flex", alignItems:"center", gap:7, fontSize:13, color:accent, textDecoration:"none", flex:"0 0 auto" }}>
                {shop.whatsapp ? (
