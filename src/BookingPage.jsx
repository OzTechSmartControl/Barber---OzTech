...
const formatBusinessHours = (raw) => {
  ...
  while (i < WEEK.length) {
    ...
    if (!cur?.enabled) {
      ...
      lines.push(`${range}: ${cur.open}-${cur.close}`);
      i = j;
    }
  }
  return lines.join(" | ");
  ...
  return (
    <div style={{ minHeight:"100vh", background:BT.bg, fontFamily:"'DM Sans',sans-serif", color:BT.text }}>
      ...
      {shop?.business_hours && (() => {
        ...
        return hoursText ? (
          <div style={{ background:BT.surface, padding:"0.5rem 1rem", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink:0 }}>
              <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
            </svg>
            <span style={{ fontSize:11, color:BT.muted }}>{hoursText}</span>
          </div>
        ) : null;
      })()}
        ...
        <div style={{ maxWidth:540, margin:"0 auto", padding:"1.5rem 1rem 1.5rem" }}>
          ...
          {step === 3 && (
            <div>
              ...
              <label style={{ display:"block", fontSize:13, color:BT.muted, fontWeight:600, marginBottom:10, textAlign: isMobile ? "center" : "left" }}>Selecione uma data</label>

              <div style={{ display:"flex", justifyContent: isMobile ? "center" : "flex-start", marginBottom:"1.5rem" }}>
                <input
                  type="date"
                  min={todayISO()}
                  value={selectedDate}
                  ...
                  style={{
                    ...inputSt,
                    colorScheme: "dark",
                    ...(isMobile ? { width:"auto", fontSize:14, padding:"0.5rem 0.75rem" } : {}),
                  }}
                />
              </div>
          ...
          <div style={{ marginTop:"1rem" }}>
            ...
            {(shop?.phone || shop?.whatsapp) && (
              <div style={{ background:BT.surface, padding:"0.5rem 1.5rem", display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"center", gap:"0.5rem 2rem" }}>
                ...
                {shop?.whatsapp && (
                  <a href={`https://wa.me/55${shop.whatsapp.replace(/\D/g,"")}`} target="_blank" rel="noopener noreferrer" style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:accent, textDecoration:"none" }}>
                ...
                {shop?.amenities?.length > 0 && (
                  <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"center", gap:"0.4rem 0.6rem" }}>
                    ...
                    {shop.amenities.map((label, i) => (
                      <span key={i} style={{ fontSize:11, padding:"2px 9px", borderRadius:99, background:`${BT.border}55`, color:BT.muted, border:`1px solid ${BT.border}` }}>{label}</span>
                    ))}
                  </div>
                )}
              </div>
