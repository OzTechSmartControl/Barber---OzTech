...
function ChartShell({ title, subtitle, icon: Icon, children }) {
  return (
    <div
      style={{
        background: "linear-gradient(180deg, rgba(26,26,36,0.98), rgba(16,17,25,0.98))",
...
function PremiumTooltip({ active, payload, label, formatter }) {
  ...
  return (
    <div style={{ background: "#10121a", border: `1px solid ${T.border}`, borderRadius: 12, padding: "0.7rem 0.85rem", boxShadow: "0 18px 40px rgba(0,0,0,.35)" }}>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 10, right: 14, left: -18, bottom: 0 }}>
          ...
          <Area type="monotone" dataKey="value" stroke={T.success} strokeWidth={3} fill="url(#revenueGradient)"
            dot={{ r: 4, strokeWidth: 2, fill: "#10121a", stroke: T.success }} activeDot={{ r: 6 }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
