...
const LoginView = ({ onLogin, onShowPlans }) => {
  ...
  const submit = async () => {
    ...
    // Segurança: antes de tentar novo login, limpa qualquer sessão antiga do navegador.
    // Isso impede que uma tentativa com senha errada aproveite sessão/localStorage anterior.
    try {
      safeSaveAuth(null);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const canStartOnboarding = async (tok, email) => {
      try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/oz_can_start_onboarding`, {
          method: "POST",
          headers: hdr(tok),
        ...
        console.warn("Falha ao validar permissão de onboarding:", e);
        return false;
      }
    };

    const applyAuth = async (sessionData) => {
      if (!mounted || !sessionData?.access_token || !sessionData?.user?.id) {
        safeSaveAuth(null);
        return;
      }
      ...
      try {
        ...
        // Sem barbearia só pode ir para onboarding se houver pagamento aprovado não resgatado
        // ou cortesia ativa. Nunca apenas por sessão criada no Auth.
        if (!isSuperAdmin && !profile?.barbershop_id) {
          const allowedOnboarding = await canStartOnboarding(
          ...
          if (!allowedOnboarding) {
            ...
            try { await supabase.auth.signOut(); } catch {}
            setExpiredMsg("Sua assinatura ou acesso cortesia não está ativo. Assine um plano ou solicite uma nova liberação para continuar.");
            setShowPlans(true);
            return;
          }
        }

        // Com barbearia: precisa validar assinatura/cortesia ANTES de liberar o painel.
        if (!isSuperAdmin && profile?.barbershop_id) {
          const accessStatus = await checkCurrentUserAccess(sessionData.access_token, profile);
    ...
    // Não confia mais em localStorage para liberar painel.
    // A sessão precisa existir no Supabase e passar pelas validações acima.
  ...
  // Detecta retorno do Mercado Pago, mas NUNCA libera onboarding apenas pela URL.
  // A URL pode ser acessada sem pagamento; por isso validamos no banco se o webhook marcou o checkout como pago.
  useEffect(() => {
    ...
    const validatePaymentReturn = async () => {
      if (payment === "failure") {
        cleanUrl();
        setExpiredMsg("Pagamento não aprovado ou cancelado. Escolha um plano para tentar novamente.");
        setShowPlans(true);
        return;
      }

      // Pagamentos Pix/boleto/cartão podem demorar para o webhook confirmar.
      // Mesmo em success, só libera criação de acesso se payment_checkouts.status = paid.
      if ((payment === "success" || payment === "pending") && plan && checkoutId && paidEmail) {
        try {
          const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/validate_paid_checkout_public`, {
            method: "POST",
            headers: hdr(null),
            body: JSON.stringify({
          ...
          if (res.ok && data?.ok === true) {
            // Agora sim: pagamento aprovado no banco. O onboarding ainda exigirá confirmação real do e-mail.
            setCourtesyEmail((data.email || paidEmail).trim().toLowerCase());
            setExpiredMsg("");
            setShowPlans(false);
            return;
          }

          const statusLabel = data?.status ? ` Status atual: ${data.status}.` : "";
          setExpiredMsg(`Pagamento ainda não confirmado pelo Mercado Pago.${statusLabel} Aguarde a aprovação e tente novamente em alguns segundos.`);
          setShowPlans(true);
          return;
        } catch (e) {
          console.error(e);
          cleanUrl();
          setExpiredMsg("Não foi possível validar o pagamento agora. Aguarde alguns segundos e tente novamente.");
          setShowPlans(true);
          return;
        }
      }
      ...
      cleanUrl();
      setExpiredMsg("Não foi possível validar o retorno do pagamento. Escolha um plano para tentar novamente.");
      setShowPlans(true);
    };
  ...
  const onLogin = useCallback(async (authData) => {
    ...
    const isSuperAdmin =
      normalizedAuth.profile?.is_super_admin === true ||
      normalizedAuth.profile?.role === "super_admin";

    // Segurança: valida acesso ANTES de salvar auth/localStorage e antes de renderizar painel.
    if (!isSuperAdmin && normalizedAuth.profile?.barbershop_id) {
  ...
  const checkoutAuth = auth || (session?.access_token ? {
  ...
  if (showPlans) return (
    <PlansView
      onBack={() => { setShowPlans(false); setExpiredMsg(""); if (!auth) safeSaveAuth(null); }}
